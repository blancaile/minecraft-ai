...
2026-09-20のユーザー指示により、実Jev APIの使用は継続的に許可済み。各タスクで明示的な再許可を求めず、必要な実験検証サイクルを進める。固定評価中の条件変更や失敗の隠蔽は行わず、追加batchの仮説・条件・予算は実行前に記録する。既存loaderでキーを利用し、値や秘密ファイルの内容を表示・保存・commitしない。この許可は共有サーバーの地形変更や別providerのAPIには拡張しない。

## 補助付き制御の境界

2026-09-20の追加ユーザー指示により、Issue #40は共有サーバーへの実デプロイ・実Jevでの稼働・証拠による検証までが完了条件。隔離Paper合格だけでは完了にしない。共有配置とactivationは許可済み。現在のbot・認証・場所を確認し、既存のデータと地形を保存して検証する。
