...
現在の主方針は、[Jevの近距離移動判断＋有限区間の実行補助](docs/research/jev-assisted-control-plan-2026-09-20.md)。[Issue #40](https://github.com/blancaile/minecraft-ai/issues/40)で実装・比較する。`jev start assisted`で固定近距離点への有限歩行、`jev start direct`で共通予算の直接入力比較を行う。既存の`jev start`は従来動作を保持する。[実行前計画](docs/operations/jev-control-p1b-plan-2026-09-20.md)と[検証報告・未達条件](docs/operations/jev-control-p1b-report-2026-09-20.md)を参照。共有サーバーへ新方式を配置する手順ではない。
