...
現在の主方針は、[Jevの近距離移動判断＋有限区間の実行補助](docs/research/jev-assisted-control-plan-2026-09-20.md)。[Issue #40](https://github.com/blancaile/minecraft-ai/issues/40)で実装・比較する。`jev start assisted`で固定近距離点への有限歩行、`jev start direct`で共通予算の直接入力比較を行う。既存の`jev start`は従来動作を保持する。[実行前計画](docs/operations/jev-control-p1b-plan-2026-09-20.md)、[隔離Paperの改善サイクル](docs/operations/jev-control-p1b-live-evaluation-2026-09-20.md)、[共有サーバーへの配置・実Jev5試行の検証](docs/operations/jev-control-p1b-shared-2026-09-20.md)、[初回実装と模擬試験](docs/operations/jev-control-p1b-report-2026-09-20.md)を参照。完了条件には共有環境での実稼働と証拠検証を含む。
